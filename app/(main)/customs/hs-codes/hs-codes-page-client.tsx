...
import { FrequentCodesListStatic } from '@/components/hs-codes/frequent-codes-list-static';
...
interface HSCodesPageClientProps {
  initialFrequentCodes: HSCode[];
}

export function HSCodesPageClient({ initialFrequentCodes }: HSCodesPageClientProps) {
  ...
  const handleCodeSelect = useCallback(async (code: HSCode | HSCodeSearchResult) => {
    setSelectedCode(code);
    // Defer duty calculation — only compute when user has selected a code
    const calc = await calculateDuties(code.hsCode, 100000000);
    setCalculation(calc);
  }, []);
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="space-y-4">
              <Card>
                ...
                <CardContent>
                  <FrequentCodesListStatic
                    codes={initialFrequentCodes}
                    onSelect={handleCodeSelect}
                  />
                </CardContent>
              </Card>
